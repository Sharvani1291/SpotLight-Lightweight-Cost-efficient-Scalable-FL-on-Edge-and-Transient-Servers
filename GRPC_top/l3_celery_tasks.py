...
from top_pb2 import ModelState
from top_pb2 import ModelWeights
from top_pb2_grpc import TopLayerAggregatorServiceStub
from spotlight_pb2 import AggregatedWeights
...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "GRPC-Edge")))
...
app = Celery('celery_tasks', backend='redis://127.0.0.1:6380/0', broker='redis://127.0.0.1:6380/0')
    ...
    try:
        ...
        if model_name in compressed_models:
            ...
            if not isinstance(model_weights, list):
                raise ValueError(f"Expected list for small model {model_name}, got {type(model_weights)}")
            
            # Convert float list to bytes
            float_bytes = np.array(model_weights, dtype=np.float32).tobytes()

            model_weights_proto = AggregatedWeights(
                model_name=model_name,
                float_weights=float_bytes,
                num_samples=weights_payload.get("num_samples"),
                version=weights_payload.get("version"),
                compressed=False
            )
        ...
        response = stub.ReceiveAggregatedWeightsFromL3(model_weights_proto)
...
def send_model_to_edge_async(weights_payload):
    try:
        ...
        logging.info(f"Preparing model weights for {model_name} to send to Edge.")

        if model_name in compressed_models:
            if not isinstance(model_weights, dict):
                raise ValueError(f"Expected state_dict for large model {model_name}, got {type(model_weights)}")

            buffer = io.BytesIO()
            torch.save(model_weights, buffer, _use_new_zipfile_serialization=False)
            raw_weights = buffer.getvalue()
            compressed_weights = bz2.compress(raw_weights)
            ...
            if not isinstance(model_weights, list):
                raise ValueError(f"Expected list of floats for small model {model_name}, got {type(model_weights)}")
        ...
        channel = grpc.insecure_channel('[::]:8083')
        stub = EdgeLayerAggregatorServiceStub(channel)
        response = stub.AggregateAndUpdateModel(model_weights_proto)

        logging.info(f"Response from Edge: {response}")
        return {"success": response.success, "message": response.message}

    except Exception as e:
        logging.error(f"Error in send_model_to_edge_async: {e}")
        raise
